      ...
      <div role="dialog" aria-label="Shopping cart" aria-modal="true"
              ...
              <div key={item.id} className="flex gap-3 items-start py-4 border-b border-warm last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => dispatch(changeQty({ id: item.id, delta: -1 }))}
                            className="w-6 h-6 rounded-full border border-warm flex items-center justify-center text-sm
                                       hover:bg-warm transition-colors bg-transparent cursor-pointer">−</button>
                ...
                {/* Remove */}
                <button onClick={() => dispatch(removeFromCart(item.id))} aria-label="Remove item"
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-warm">
            ...
            <button
              onClick={() => { dispatch(closeCart()); import('react-router-dom').then(m => m.useNavigate ? null : null); window.location.href='/checkout'; }}
